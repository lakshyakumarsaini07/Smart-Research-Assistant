...
from app.utils.llm_interface import TogetherLLM
from app.modules.vector_store import VectorStore
from app.modules.summarizer import LangchainTogetherLLM

def create_qa_model() -> LangchainTogetherLLM:
    """
    Create an instance of the QA model
    
    Returns:
        LangchainTogetherLLM model instance
    """
    # Create TogetherLLM with higher temperature for more diverse answers
    return LangchainTogetherLLM(temperature=0.7)
...
def answer_question(qa_chain: RetrievalQA, question: str) -> Dict[str, Any]:
    ...
    # Get the first source document if available
    source = None
    if source_docs:
        source = source_docs[0].page_content
        if len(source) > 300:
            source = source[:300] + "..."
...
class ConversationalQA:
    ...
    def answer(self, question: str) -> Dict[str, Any]:
        ...
        # Generate answer
        llm_instance = TogetherLLM(temperature=0.7)
